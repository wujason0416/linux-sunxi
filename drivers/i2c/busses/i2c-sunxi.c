...
*/
static void aw_twi_set_clock(unsigned int clk_in, unsigned int sclk_req,
			     void *base_addr)
{
...
static int i2c_sunxi_do_xfer(struct sunxi_i2c *i2c, struct i2c_msg *msgs,
			     int num);
...
static int aw_twi_stop(void *base_addr)
{
	...
	timeout = 0xff;
	while ((TWI_STAT_IDLE != readl(base_addr + TWI_STAT_REG)) &&
	       (--timeout))
		;
	if (timeout == 0) {
...
static int i2c_sunxi_core_process(struct sunxi_i2c *i2c)
{
	...
	if (i2c->msg == NULL) {
		pr_err("i2c->msg is NULL, err_code = 0xfe\n");
		err_code = 0xfe;
		goto msg_null;
	}
...
static int i2c_sunxi_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs,
			  int num)
{
...
static int i2c_sunxi_probe(struct platform_device *dev)
{
	...
	ret = request_irq(irq, i2c_sunxi_handler, IRQF_DISABLED,
			  i2c->adap.name, i2c);
	if (ret)
		goto ereqirq;
