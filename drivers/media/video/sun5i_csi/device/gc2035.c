...
#define  VGA_WIDTH		640//800
#define  VGA_HEIGHT 	       480



unsigned int   temp_2035=0x00,temp_2035_exp=0x00;
/*
...
{{0xa2},{0x80}},
	...
	{{0x84},{0x45}},//RB offset
	{{0xd1},{0x42}},  //saturation_cb  0x38
	{{0xd2},{0x42}},  //saturation_Cr  0x38
             ...
             struct v4l2_mbus_framefmt *fmt)//linux-3.0
{
	...
	if((wsize->width==1600)&&(wsize->height==1200))  //capture mode  >640*480
	{
		...
		sensor_read(sd, regs.reg_num, regs.value);
	              ...
	              printk("  111  regs.value = 0x%x\n", regs.value[0]);


		temp |= (regs.value[0]<< 8);


	              printk("  222  temp = 0x%x\n", temp);

		regs.reg_num[0] = 0x04;
		sensor_read(sd, regs.reg_num,regs.value);

	       printk("  3333  regs.value = 0x%x\n", regs.value[0]);
		...
		shutter=temp;
	      ...
	      temp_2035=0x01;
	  	temp_2035_exp=shutter;


		printk("55555  shutter = [%x]\n", shutter);
